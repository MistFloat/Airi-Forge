...
import { evaluateSpeechSegment } from './speech-segment-policy'
...
export class VAD implements BaseVAD {
  ...
  private voicedSampleCount: number = 0
  ...
  public async processAudio(inputBuffer: Float32Array): Promise<void> {
    ...
    if (isSpeech)
      this.voicedSampleCount += inputBuffer.length
  ...
  private processSpeechSegment(overflow?: Float32Array): void {
    ...
    const decision = evaluateSpeechSegment({
      minSpeechDurationMs: this.config.minSpeechDurationMs,
      sampleRate: this.config.sampleRate,
      segmentSamples: this.bufferPointer,
      voicedSamples: this.voicedSampleCount,
    })
    if (!decision.shouldUpload) {
      this.emit('speech-rejected', {
        duration,
        reason: decision.reason!,
        voicedDurationMs: decision.voicedDurationMs,
        voicedRatio: decision.voicedRatio,
      })
      if (overflow)
