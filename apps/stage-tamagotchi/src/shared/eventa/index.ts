...
/**
 * Progress pushed from the main process while an MCP tool call is executing.
 *
 * Use when:
 * - A server reports `notifications/progress` for a long-running tool
 * - Chat UI wants to show live progress on the matching tool-call block
 *
 * Expects:
 * - `name` is the qualified tool name (`serverName::toolName`) used for invocation
 *
 * Returns:
 * - N/A
 */
export interface ElectronMcpToolProgressPayload {
  message?: string
  /** Qualified tool name (`serverName::toolName`) the progress belongs to. */
  name: string
  progress: number
  total?: number
}
export const electronMcpToolProgress = defineEventa<ElectronMcpToolProgressPayload>('eventa:event:electron:mcp:tool-progress')
