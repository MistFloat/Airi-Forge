...
function alignWindow(window: BrowserWindow, position: 'bottom-left' | 'bottom-right' | 'center' | 'top-left' | 'top-right'): void {
  const { height: windowHeight, width: windowWidth } = window.getBounds()
  const { height: areaHeight, width: areaWidth, x: areaX, y: areaY } = screen.getPrimaryDisplay().workArea
